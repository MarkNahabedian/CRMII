...
convert_sj_timestamp(t) = unix2datetime(t/1000)

# ╔═╡ 1e8ea2a6-ec63-402c-92e0-0f8632059e0f
const TickTockData = Vector{
    NamedTuple{(:timestamp, :interval, :sound),
               Tuple{DateTime,
                     Union{Nothing,
                           Millisecond},
                     Float64}}}
...
select(data::TickTockData, column::Symbol) =
    map(data) do t
        getfield(t, column)
    end
...
begin
	data_file = "tik-tok-2022-11-10.csv"
    DATA = Vector{
        NamedTuple{(:timestamp, :interval, :sound),
                   Tuple{DateTime, Union{Nothing, Millisecond}, Float64}}}()
    previous_timestamp = nothing
    for (a, b) in CSV.File(data_file)
        global previous_timestamp
        ts = convert_sj_timestamp(a)
        delta = if previous_timestamp === nothing
            nothing
        else
            ts - previous_timestamp
        end
        push!(DATA, (timestamp = ts,
                     interval = delta,
                     sound = b))
...
function show_stats(column, description)
    println(description)
    s = stats(column)
...
let
    show_stats(map(x -> x.value,
               ...
               "Time sampling interval (milliseconds)")
	println()
    show_stats(select(DATA, :sound), "Sound")
end
...
FILTERED_SOUND = let
	...
	s =stats(select(DATA, :sound))
	k = ImageFiltering.Kernel.gaussian((s.standard_deviation,))
	imfilter(select(DATA, :sound), k)
end
...
PEAK_TIMESTAMPS =
let
	timestamps = select(DATA, :timestamp)
	map(sort(findpeaks(FILTERED_SOUND)))do index
		timestamps[index]
	end
end
...
Findpeaks = "d9b5fb6a-56ed-4fb5-a628-2e4d0535521b"
...
[[deps.Findpeaks]]
deps = ["Test"]
git-tree-sha1 = "e93b932f1f6704caa99c818df73e045a95257ea7"
repo-rev = "master"
repo-url = "https://github.com/tungli/Findpeaks.jl"
uuid = "d9b5fb6a-56ed-4fb5-a628-2e4d0535521b"
version = "0.1.0"
